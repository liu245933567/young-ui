...
<script>
import CascadePicker from '@packages/cascade-picker';
import visibilityMixin from '@mixins/visibility';
import popupMixin from '@mixins/popup';
import pickerMixin from '@mixins/picker';
import { deepAssign, findIndex } from '@utils/helpers/util';
import { computeNatureMaxDay, formatType } from '@utils/lang/date';

const COMPONENT_NAME = 'jt-date-picker';
...
export default {
  name: COMPONENT_NAME,
  components: {
    [CascadePicker.name]: CascadePicker
  },
    ...
    _generateData(i, count, item) {
      ...
      if (count < this.columnCount - 1 && i < 5) {
        (item.children || item).forEach((subItem) => {
          (!subItem.children || subItem.isMin || subItem.isMax) &&
