<template>
  <div
    v-if="type === 'data'"
    class="jt-picker-slot"
    :style="`flex: ${flex};text-align: ${textAlign};`"
  ...
  <div
    ...
    class="jt-picker-slot jt-picker-slot-divider"
    :style="`flex: ${flex ? flex : 'none'};text-align: ${textAlign};`"
    :class="{ 'jt-picker-slot-hidden': !ready }"
  >
    <p>{{ content }}</p>
  </div>
  ...
  methods: {
    ...
    bindEvents() {
      draggable(this.$el, {
        start: (event) => {
          ...
          this.dragState.initialTranslateY = translateUtils.getElementTranslate(
            this.$el
          ).top;
        },
        drag: (event) => {
          ...
          this.dragState.endTop =
            this.dragState.offsetTop + this.dragState.initialTranslateY;
          translateUtils.translateElement(
            this.$el,
            null,
            this.dragState.endTop
          );
          ...
          let Index = Math.round(
            (this.dragRange.top - this.dragState.endTop) / this.itemHeight
          );
          this.locateItem(Index);
          this.dragState = {};
        },
    ...
    locateItem(Index) {
      ...
      translateUtils.translateElement(
        this.$el,
        null,
        this.itemHeight * ((this.showItemCount - 1) / 2 - Index)
      );
...
<style lang="scss">
$slot-text-size: .56rem /* 42/75 */;
$selected-text-color: #404040;
$basic-text-color: #b3b3b3;
$picker-slot-item-padding: .106667rem /* 8/75 */;
...
.jt-picker-slot {
  ...
  &-item {
    ...
    overflow: hidden;
    padding: 0 $picker-slot-item-padding;
    &-item-selected {
      color: $selected-text-color;
    }
  }
